...
from PyQt6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout, 
                             ...
                             QMenu, QInputDialog) # 新增导入 QMenu, QInputDialog
...
class MainWindow(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('夏令营申请材料生成器 by 叶影 (v3.1 - 集成文件操作)')
        self.resize(900, 700)
    ...
    def show_context_menu(self, position):
        """当在列表上右键单击时，创建并显示菜单"""
        list_widget = self.sender() # 获取信号的来源（是哪个列表）
        ...
        action_rename = QAction("重命名", self) # 重命名没有特别合适的标准图标
    ...
    def preview_file(self, item):
        ...
        if not file_path or not os.path.exists(file_path):
            ...
            return
        try:
            os.startfile(file_path)  # os.startfile 在Windows上表现良好
        except Exception as e:
        ...
        try:
            ...
            for i, path in enumerate(converted_pdf_paths):
                try:
                    ...
                    current_page_in_content += len(reader.pages)
            ...
            content_merger.write(content_path)
